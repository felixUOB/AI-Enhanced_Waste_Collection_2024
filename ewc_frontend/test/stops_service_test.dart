...
void main() {
  ...
  group('StopsService.fetchAllStops', () {
      test('Returns a list of Stop on success if stop is due', () async {
        ...
        when(getIt<AuthService>().makeAuthenticatedRequest('stops/'))
            .thenAnswer((_) async => mockResponse);
        ...
        expect(stops.first.id, 3);
        expect(stops.first.name, 'Stop C');
        expect(stops.first.location.latitude, 52.2);
      });
